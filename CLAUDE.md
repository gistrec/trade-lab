...
## Project context
Python crypto-spot backtester that now runs a live trading system. Solo developer in Serbia, test capital <$10k. Real money target is **Binance mainnet** (the earlier "Binance is geo-blocked in Serbia, real money goes to Kraken" assumption was wrong — corrected by the user 2026-07-09; the Kraken migration plan is cancelled).

## Current phase
execution-layer #2b is complete (production daily CLI `paper-place-orders` with deterministic clientOrderId idempotency, reconstruction, wait-for-ack, schema v2 journal). Mainnet support shipped 2026-07-09: testnet and mainnet run side by side (symmetric `.env.testnet` (default) / `.env.mainnet` (via `--env-file`), per-environment journal/state with runtime isolation guards, dashboard source switcher, three-flag order gate). The rollout ladder is **complete**: testnet pipeline validation → mainnet read-only observation (6-hourly dry-run, key without trading permission — mainnet has full kline history, so this is where the SMA(200)/ladder signal path was actually validated; testnet structurally cannot, it wipes candles ~monthly) → capped smoke test → daily live cron. Now: **LIVE on Binance mainnet with real money since 2026-08-24**, ~148 USDT, daily `paper-place-orders` cron; the first live cycle filled 7/7 basket legs. The forward test is running, not passed — the live sample is weeks old and proves the pipeline, not the edge. See `src/trade_lab/execution/README.md` § "Mainnet (real money)".

## Deployable strategy
TSMOM lookbacks `(28, 60)` on an **equal-weight market-basket index** of 7 assets: BTC, ETH, BNB, SOL, ADA, XRP, DOGE. SMA(200) regime gate on the basket close. Pro-rata ladder signal `{0, 0.5, 1.0}` (average of binary per-lookback states). Basket rebalances monthly (`freq="MS"`) or when `N_active` changes. Concat-OOS Sharpe +1.48 (venue-verified *replay* window 0.72 — a fixed-config historical replay of the frozen config over 2022-01-21 → 2026-05-27 to check venue agreement; **not** selection-OOS, **not** forward data); DSR under the project's own conservative deflator (pinned conservative assumption `sd_trial_sharpes ≈ 0.7` annualized, `0.7/sqrt(365)` per-period — a-priori, no trial panel) = 0.037 ≈ 0 — the deploy case rests on parameter stability (the deployed config **plus 6 neighbor configs** — 7 cluster members, so six confirmations, not seven; the deployed point is not evidence for itself — all in the raw concat-OOS Sharpe band +1.37…+1.49, effectively one return stream; no lone peak) plus the forward test. DSR 0.770 for the deployed config and the cluster's threshold form (median 0.736 over all 7 members, 6/6 neighbors > 0.5) are secondary figures under the 1/sqrt(T) convention, which is also the convention the project's deploy gate is defined on — the gate was not re-run when reporting moved to the conservative deflator (`findings/dsr_convention_2026_08.md`, RESULTS.md § "What LIVE status actually means"). Composition of the basket is an ex-post choice: the survivorship/composition axis is still open (`findings/validation_universe_bias.md` addendum 2026-08-25). Low turnover by design.
...
Last updated: 2026-08-30 (phase corrected to LIVE on mainnet — the file still described the read-only observation step that finished on 2026-08-24). Update when phase changes or hard rules are revised.
